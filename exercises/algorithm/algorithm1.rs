...
impl<T> LinkedList<T> {
        ...
        T: std::cmp::PartialOrd + Clone,
    {
        ...
        while node_a.is_some() && node_b.is_some() {
            let a = unsafe { node_a.unwrap().as_ref().val.clone() };
            let b = unsafe { node_b.unwrap().as_ref().val.clone() };
            if a < b {
                merged_list.add(a);
        ...
        while node_a.is_some() {
            merged_list.add(unsafe { node_a.unwrap().as_ref().val.clone() });
            node_a = unsafe { node_a.unwrap().as_ref().next }
        }

        while node_b.is_some() {
            merged_list.add(unsafe { node_b.unwrap().as_ref().val.clone() });
            node_b = unsafe { node_b.unwrap().as_ref().next }
        }
